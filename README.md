...
- `setOrderPayment(fied: keyof TOrderPayment, value: string): void` - получить данные о оплате
- `setOrderContacts(fied: keyof TOrderContacts, value: string): void` - получить данные о контактах
- `validatePayment(): void` - проверка на валидность формы оплаты
- `validateContacts(): void` - проверка валидности формы контактов
- `hasProduct(TProductId): void` - проверка на то имеется ли данный товар в корзине
...
- `itemId: string` - id товара
- `count: HTMLElement` - элемент карточки порядковый номер
- `title: HTMLElement` - элемент карточки название
- `price: HTMLElement` - элемент карточки цена
...
Расширяет класс Component. Предназначен для реализации главной страницы с карточками товара и иконкой корзины. Конструктор принимает HTMLElemnt в котором будет разметка содержимого, экземпрял класса EventEmitter для возможности инициировать события
...
Расширяет класс Component. Оповещает об успешной покупке и выводит общую сумму заказа с помощью метода `show()`

> > > > > > > 86b954f25b44f25a7b7820dbe7d9e0714ce6be13
